...
### 데이터 모델 참고 (필드 9·10 확정 반영)

알림 테이블의 시각 관련 컬럼은 최종적으로 다음 두 개로 확정됐다. 별도 컬럼을 더 추가하지 않는다.

| 컬럼 | 값이 채워지는 대상 | 용도 | 갱신 시점 |
|---|---|---|---|
| `group_created_at` (필드 9) | **좋아요 알림에만** 값이 들어간다. 댓글·블라인드는 `NULL` | 4시간 안전장치 계산 전용 | 그룹이 새로 시작될 때 1회, 이후 불변 |
| `created_at` (필드 10, 알림 생성 일시) | 세 종류 모두 값이 들어간다 | **목록 정렬 기준.** 좋아요는 그룹으로 묶이므로, 최신순 정렬을 위해 그룹에 새 좋아요가 들어올 때마다 가장 최근 알림의 생성 일시로 갱신된다 | 최초 생성 시, 그리고 좋아요 그룹에 새 좋아요가 추가될 때마다 |

즉 `created_at` 하나가 "생성 시각"이면서 동시에 "최근 활동 시각" 역할을 겸한다 — 좋아요는 그룹이 갱신될 때마다 이 값이 최신 좋아요 시각으로 덮어써지고, 댓글·블라인드는 그룹핑이 없어 최초 생성 시각에서 변하지 않는다. 이전 초안에서 별도로 뒀던 `last_activity_at` 컬럼은 필요 없어져 제거했다.

`group_created_at`은 좋아요 알림에만 값이 있고 나머지는 `NULL`이므로, 목록 정렬이나 커서에는 절대 쓰지 않는다 — 4시간 안전장치 판정 로직에서만 참조한다.
